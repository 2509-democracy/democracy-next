...
export async function evaluateHackathon({
  ...
  console.log(
    `モックAI評価: テーマ「${theme}」, アイデア「${
      idea.trim() || "アイデア未入力"
    }」, 技術: ${techNames.join(", ")}, スコア: ${Math.floor(finalScore)}`
  );

  // 評価内容のテキスト（穴埋め形式で模擬出力）
  const responseText =
    `SCORE: ${Math.floor(finalScore)}\n` +
    `テーマへの合致度: 10/10点 理由: テーマ「${theme}」に沿ったアイデア\n` +
    `技術チャレンジ: ${techBonus}/30点 理由: 技術数=${techNames.length}\n` +
    `技術の妥当性: 30/30点 理由: 技術選定妥当\n` +
    `技術的実現性: 25/30点 理由: 実現可能性高\n`;
